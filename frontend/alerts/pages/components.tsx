...
import { createOptionKey, formatTextWithHighlights, getTextFromItem } from './utils';
  ...
  <div className="w-full lg:w-64 p-4 bg-white shadow rounded-md h-fit sticky top-4">
    ...
    <div className="mb-4">
      ...
      <select
        ...
        onChange={(e) => onFilterChange('school', e.target.value)}
        className="w-full p-2 border rounded text-sm bg-white border-[#C0D5DE] border-[1.6px]"
        disabled={!state.options.schoolOptions.length || state.loading.isInitialLoad}
    ...
    <div className="mb-4">
      ...
      <select
        ...
        onChange={(e) => onFilterChange('grade', e.target.value)}
        className="w-full p-2 border rounded text-sm bg-white border-[#C0D5DE] border-[1.6px]"
        disabled={!state.options.gradeOptions.length || state.loading.isInitialLoad}
 ...
 */
export const SummaryCards: React.FC<SummaryCardsProps> = ({ analysisData, onDownloadReport }) => {
  const cardConfigs = [
    {
 ...
 */
const InsightPanel: React.FC<InsightPanelProps> = ({ title, icon, items, emptyMessage }) => (
      ...
      {items?.length > 0 ? (
        <ul className="space-y-3" role="list">
          {items.map((item, index) => {
            ...
            return (
              <li key={index} className="text-gray-800 text-sm leading-relaxed" role="listitem">
                <div className="flex items-start">
                  <span className="text-teal-600 mr-2 mt-1" aria-hidden="true">•</span>
 ...
 */
export const InsightsAndRecommendations: React.FC<InsightsAndRecommendationsProps> = ({ analysisData }) => (
 ...
 */
export const StatusNotifications: React.FC<StatusNotificationsProps> = ({ state }) => (
  <>
    {/* Error Notification */}
